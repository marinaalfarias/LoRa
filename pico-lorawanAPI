...
```
### ABP
Inicializar a biblioteca para ABP
```c
// ABP settings
...
- `sx1276_settings` - ponteiro para configurar for SX1276 SPI and GPIO pins
- `region` - região to usar, see [`enum LoRaMacRegion_t
...
- `abp_settings` - ponteiro to LoRaWAN ABP settings

Returns `0` on success, `-1` on error.

### OTA
Inicializa a biblioteca para OTAA
...
Returns `0` on success, `-1` on error.
### Joins Status
Consulta o status da rede LoRaWAN
```c
int lorawan_is_joined();
```
Returns`1` se é bem sucedido e `0`caso contrário.
